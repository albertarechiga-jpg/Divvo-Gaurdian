// Server-only: the single action that actually authorizes cargo release.
// api/submit-bol.js creates a BOL as "pending_verification" with two real
// captured photos and nothing else — this endpoint is where an admin
// (deliberately admin-only, not dispatcher-or-above like every other write
// this feature touches) looks at those photos and decides. Approve flips
// the BOL to "signed_pickup"; reject flips it to "verification_rejected".
// Both write a chain-of-custody event describing what was actually decided,
// not what was attempted — "pickup" is only ever logged here, once release
// is actually authorized, never at submission time.
...
export default async function handler(req, res) {
  ...
  if (!verificationId || !["approve", "reject"].includes(decision)) {
    return res.status(400).json({ error: "Missing verificationId or decision must be 'approve' or 'reject'" });
  }
  ...
  try {
    ...
    // 2. Confirm the caller is specifically an admin — authorizing cargo
    //    release is a deliberately higher bar than dispatcher-or-above,
    //    which is what every other write in this feature requires.
    ...
    // 3. Load the verification and confirm it belongs to the caller's org,
    //    via driver -> carrier -> organization.
    ...
    // 4. Find the BOL this verification's signature is attached to.
    ...
    // 5. Record the decision on the verification itself.
    ...
    // 6. Flip the BOL's status to match.
    ...
    // 7. Chain-of-custody entry — "pickup" only ever gets logged here, at
    //    the moment release is actually authorized, not at submission.
