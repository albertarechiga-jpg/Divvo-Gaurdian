...
There is a single recovery-case UI, `RecoveryDetail.jsx`, keyed by `incidentId` and backed by shared `incidents`/`recoveryDetails` state in `App.jsx`. It's reachable two ways: `Recovery.jsx` (the sidebar "Recovery" list) → click a case, or `UnifiedCommandCenter.jsx`'s per-device "Recovery Actions" button, which bridges the device into a real incident on demand (`createIncidentForDevice()` in `detectionEngine.js`, `buildDefaultRecoveryDetailForDevice()` in `recoveryMock.js`, wired through `App.jsx`'s `handleViewOrCreateDeviceIncident` + `deviceIncidents` map so repeat clicks on the same device reuse its incident instead of creating duplicates). An earlier, separate device-keyed page (`RecoveryCase.jsx`, its own hardcoded mock data, no connection to `incidents`) was deleted in this consolidation — if you see it referenced anywhere, it's stale.

Incidents without a matching `SHIPMENTS` record (i.e. device-originated ones) carry `deviceId`/`trailerId` instead of `shipmentId`, and `recoveryDetails[id].deviceSnapshot` (door/lock/battery/camera/vibration/lte at creation time) instead of relying on a live shipment record — `RecoveryDetail.jsx` renders whichever one is present.
...
`RecoveryDetail.jsx`'s "Contact" actions (Contact Carrier, Contact Agency, Contact Adjuster) are real too, but through a different channel than `dispatchAlert()`: they open a pre-filled `mailto:`/`tel:` link in the operator's own mail/phone app, using per-case contact data (`recoveryMock.js`'s `insurance`/`lawEnforcement` objects, `shipments.js`'s `CARRIER_CONTACTS`). Contact Agency has a fallback for cases with no assigned agency yet (`le.contactPhone === "—"`, true for every device-originated case): reverse-geocodes the case's last GPS fix and matches it against the company's configured `alert_settings.le_contacts` jurisdiction directory to auto-fill a recipient, falling back to a blank "To" field when no jurisdiction match exists.

### Map rendering

`mapbox-gl` is used directly (imported in `UnifiedCommandCenter.jsx`, CSS loaded dynamically) with `MAPBOX_TOKEN` from `src/lib/mapbox.js` (see above). `src/lib/mapbox.js` also exports `geocode()` (forward) and `reverseGeocode()` (coordinates → city/county/state, used by `RecoveryDetail.jsx`'s law-enforcement jurisdiction lookup).
